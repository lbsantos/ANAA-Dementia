...
class ProbabilisticTagger(object):
    ...
    def __read_mac_morpho(self, uppercase):
        ...
        word_tag_list = dict()
        folder_path = "../resource/lexical/mac_morpho"
        files = [file_text for file_text in os.listdir(folder_path)
                 if file_text != 'README']

        for file_name in files:
            data_file = codecs.open(os.path.join(folder_path, file_name),
                                    'r',
                                    encoding='latin1')
            ...
            for line in lines:
                ...
                if len(tmp_list) == 2:
                    ...
                    if not word in word_tag_list:
                        word_tag_list[word] = dict()
                    if not tag in word_tag_list[word]:
                        word_tag_list[word][tag] = 0
                    word_tag_list[word][tag] += 1

        for word, tags in word_tag_list.items():
            word_tag_list[word] = sorted(tags.items(),
    ...
    def __read_delaf(self):
        ...
        word_tag_list = dict()
        data_file = codecs.open("../resource/lexical/DELAF_PB.dic",
                                'r',
                                encoding='utf-16')
        lines = data_file.readlines()
